...
def abrir_arquivo_crossplataforma(path):
    import platform
    import subprocess
    import os
    if platform.system() == "Windows":
        abrir_arquivo_crossplataforma(path)
...
class AssistenteProjeto(QDialog):
    ...
    def criar_projeto(self):
        ...
        try:
            ...
            self.projeto_criado.emit(caminho_projeto)
            self.close()

        except subprocess.CalledProcessError as e:
            QMessageBox.critical(self, "Erro", f"Erro ao criar ambiente virtual:\n{e}")
...
class GerenciadorBibliotecas(QWidget):
    ...
    def listar_bibliotecas_instaladas(self):
        ...
        bibliotecas = resultado.stdout.splitlines()[2:]
        self.bibliotecas_instaladas = bibliotecas
    ...
    def executar_comando_pip(self, comando_args):
        """Executa um comando pip no terminal do macOS."""
        comando_pip = "python3 -m pip " + " ".join(comando_args)
        comando_osascript = f'tell app "Terminal" to do script "cd {os.getcwd()} && {comando_pip}"'
        try:
            subprocess.run(["osascript", "-e", comando_osascript])
        except Exception as e:
            QMessageBox.critical(self, "Erro", f"Erro ao executar pip:\n{e}")
...
class IDE(QMainWindow):
    def __init__(self):
        ...
        splitter.addWidget(self.aba_arquivos)
        splitter.addWidget(self.abas)
        ...
        layout_principal.addWidget(self.botao_executar)
    ...
    def abrir_assistente_projeto(self):
        ...
        assistente.projeto_criado.connect(self.carregar_novo_projeto)
        assistente.exec_()
    ...
    def executar_codigo(self):
        ...
        if isinstance(editor_atual, QPlainTextEdit):
            codigo = editor_atual.toPlainText()
            script_path = os.path.join(os.getcwd(), "temp_script.py")
            with open(script_path, "w", encoding='utf-8') as arquivo_temp:
                arquivo_temp.write(codigo)
            try:
                comando = f'tell app "Terminal" to do script "cd {os.getcwd()} && python3 temp_script.py"'
    ...
    def criar_menu(self):
        ...
        acao_abrir_pasta.triggered.connect(self.abrir_pasta)
        ...
        for nome, chave in temas.items():
            acao_tema = QAction(nome, self)
            acao_tema.triggered.connect(lambda _, k=chave: self.definir_tema(k))
            menu_visualizar.addAction(acao_tema)
    ...
    def abrir_arquivo(self):
        ...
        if nome_arquivo:
            ...
            self.abas.addTab(novo_editor, os.path.basename(nome_arquivo))
            self.abas.setCurrentWidget(novo_editor)
    ...
    def salvar_arquivo(self):
        ...
        if isinstance(editor_atual, EditorCodigo):
            ...
            if nome_arquivo:
                ...
                self.abas.setTabText(self.abas.currentIndex(), os.path.basename(nome_arquivo))
    ...
    def verificar_codigo_com_flake8(self):
        self._salvar_codigo_temp()
        try:
            comando = f'tell app "Terminal" to do script "cd {os.getcwd()} && python3 -m flake8 temp_script.py"'
            subprocess.run(["osascript", "-e", comando])
        except Exception as e:
            QMessageBox.critical(self, "Erro", f"Erro ao executar Flake8:\n{e}")

    def verificar_codigo_com_pylint(self):
        self._salvar_codigo_temp()
        try:
            comando = f'tell app "Terminal" to do script "cd {os.getcwd()} && python3 -m pylint temp_script.py"'
            subprocess.run(["osascript", "-e", comando])
        except Exception as e:
            QMessageBox.critical(self, "Erro", f"Erro ao executar Pylint:\n{e}")

    def verificar_codigo_com_mypy(self):
        self._salvar_codigo_temp()
        try:
            comando = f'tell app "Terminal" to do script "cd {os.getcwd()} && python3 -m mypy temp_script.py"'
            subprocess.run(["osascript", "-e", comando])
        except Exception as e:
            QMessageBox.critical(self, "Erro", f"Erro ao executar Mypy:\n{e}")

    def _salvar_codigo_temp(self):
        editor_atual = self.abas.currentWidget()
        if isinstance(editor_atual, QPlainTextEdit):
            with open("temp_script.py", "w", encoding='utf-8') as f:
                f.write(editor_atual.toPlainText())
